...
describe('CLBool', () => {
  ...
  it('toBytes() / fromBytes() do proper bytes serialization', () => {
    ...
    const fromBytes1 = CLValueParsers.fromBytes(
      myBoolBytes,
      new CLBoolType()
    ).unwrap();
    const fromBytes2 = CLValueParsers.fromBytes(
      myBool2Bytes,
      new CLBoolType()
    ).unwrap();
    ...
    expect(
      CLValueParsers.fromBytes(Uint8Array.from([9, 1]), new CLBoolType()).ok
    ).to.be.eq(false);
    expect(
      CLValueParsers.fromBytes(Uint8Array.from([9, 1]), new CLBoolType()).val
    ).to.be.eq(CLErrorCodes.Formatting);
    expect(
      CLValueParsers.fromBytes(Uint8Array.from([]), new CLBoolType()).ok
    ).to.be.eq(false);
    expect(
      CLValueParsers.fromBytes(Uint8Array.from([]), new CLBoolType()).val
    ).to.be.eq(CLErrorCodes.EarlyEndOfStream);
  });
